...
/// @brief K-nearest neighbor search setting for 2D.
struct KnnSetting2D {
public:
...
/// @brief K-nearest neighbor search result container for 2D.
/// @tparam N   Number of neighbors to search. If N == -1, the number of neighbors is dynamicaly determined.
template <int N, typename IndexTransform = identity_transform_2d>
struct KnnResult2D {
public:
  ...
  /// @param index_transform  Index transformation function (e.g., local point index -> global cell + point index)
  explicit KnnResult2D(
    size_t* indices,
    double* distances,
    int num_neighbors = -1,
...
/// @brief Radius neighbor search result container for 2D.
template <typename IndexTransform = identity_transform_2d>
struct RadiusSearchResult2D {
public:
  /// @brief Constructor
  /// @param index_transform  Index transformation function (e.g., local point index -> global cell + point index)
  explicit RadiusSearchResult2D(const IndexTransform& index_transform = IndexTransform()) : index_transform(index_transform) { neighbors.reserve(32); }
