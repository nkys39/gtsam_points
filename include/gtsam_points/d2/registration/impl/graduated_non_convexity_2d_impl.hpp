...
#pragma omp parallel for num_threads(params.num_threads) schedule(guided, 4)
  ...
  if (params.tuple_check && correspondences.size() > params.max_num_tuples * 3) {
    ...
    for (int i = 0; i < params.max_num_tuples; i++) {
      ...
      for (int k = 0; k < 3; k++) {
        ...
        // For 2D: compute distance using 2D part (x, y)
        const double d1 = (frame::point(target, c1.first).head<2>() - frame::point(target, c2.first).head<2>()).norm();
        const double d2 = (frame::point(source, c1.second).head<2>() - frame::point(source, c2.second).head<2>()).norm();
  ...
  // Rough estimate of the diameter of the target point cloud (2D)
  Eigen::Array3d min_pt = frame::point(target, 0);
  Eigen::Array3d max_pt = frame::point(target, 0);
  ...
  // Use 2D distance for diameter
  double mu = (max_pt.head<2>() - min_pt.head<2>()).matrix().norm();

  // GNC loop
  const double inlier_thresh_sq = std::pow(2.0 * params.max_corr_dist, 2);

  RegistrationResult2D result;
  ...
  for (int i = 0; i < params.max_iterations; i++) {
    for (int j = 0; j < params.innter_iterations; j++) {
      ...
      std::vector<Eigen::Vector3d> target_points(correspondences.size());
      std::vector<Eigen::Vector3d> source_points(correspondences.size());
      ...
      for (int j = 0; j < correspondences.size(); j++) {
        ...
        const Eigen::Vector3d transformed = result.T_target_source * source_pt;
        const Eigen::Vector3d residual = target_pt - transformed;

        // 2D squared norm (only x, y)
        const double error = residual.head<2>().squaredNorm();
      ...
      // SE(2) alignment
      result.T_target_source = align_points_se2(target_points.data(), source_points.data(), weights.data(), weights.size());
