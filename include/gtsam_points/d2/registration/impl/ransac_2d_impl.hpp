...
RegistrationResult2D estimate_pose_ransac_2d_(
  ...
  const RANSACParams2D& params) {
  //
  const double inv_resolution = 1.0 / params.inlier_voxel_resolution;

  FastOccupancyGrid2D target_voxels(params.inlier_voxel_resolution);
  target_voxels.insert(target);

  // Sample random source indices (need 2 points for SE(2))
  ...
  // Calculate a transformation from source to target based on the sampled indices (SE(2))
  const auto calc_T_target_source = [&](const auto& source_indices, const auto& target_indices) {
    return align_points_se2(
  ...
  const auto count_inliers = [&](const Eigen::Isometry2d& T_target_source) {  //
    return target_voxels.calc_overlap(source, T_target_source);
  };

  const int num_samples = 2;  // SE(2) requires 2 point pairs

  std::mutex mutex;
  std::atomic_uint64_t seed = params.seed;
  std::atomic_bool early_stop = false;
  std::atomic_uint64_t best_inliers = 0;
  Eigen::Isometry2d best_T_target_source = Eigen::Isometry2d::Identity();
  ...
  const auto perpoint_task = [&] {
    ...
    const Eigen::Isometry2d T_target_source = calc_T_target_source(source_indices, target_indices);
    for (const auto& taboo : params.taboo_list) {
      const Eigen::Isometry2d delta = taboo.inverse() * T_target_source;
      const double delta_r = std::atan2(delta.linear()(1, 0), delta.linear()(0, 0));  // Extract rotation angle for SE(2)
      const double delta_t = delta.translation().norm();
      if (std::abs(delta_r) < params.taboo_thresh_rot && delta_t < params.taboo_thresh_trans) {
        return;
      }
    }
...
#pragma omp parallel for num_threads(params.num_threads) schedule(guided, 4)
  ...
  return RegistrationResult2D{best_inliers / static_cast<double>(frame::size(target)), best_T_target_source};
}
