...
class SlamWithCTICPNode : public rclcpp::Node
{
public:
  SlamWithCTICPNode()
  : Node("slam_with_ct_icp"),
    key_counter_(0),
    initialized_(false)
  {
    // Declare parameters
    ...
    scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
      scan_topic_, 10,
      std::bind(&SlamWithCTICPNode::scanCallback, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(this->get_logger(), "CT-ICP SLAM Node initialized");
    RCLCPP_INFO(this->get_logger(), "Continuous-Time ICP for motion compensation");
  }
...
private:
  void scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
  {
    ...
    // Add new keyframe with CT-ICP
    addKeyframeWithCTICP(scan, msg->header.stamp);
  ...
  /**
   * @brief Convert LaserScan to PointCloud2D with per-point timestamps
   *
   * Each point's timestamp is calculated as: time_increment * point_index
   * Timestamps are normalized to [0, 1] range for CT-ICP
   */
    ...
    const rclcpp::Time& stamp)
  {
    ...
    RCLCPP_INFO(this->get_logger(), "First scan initialized with CT-ICP");
  }

  void addKeyframeWithCTICP(
    const std::shared_ptr<PointCloud2DCPU>& scan,
    const rclcpp::Time& stamp)
  {
    // Get target keyframe (last keyframe)
    auto target = keyframes_.back();

    // Initial guess: constant velocity model
    // The scan starts where the last scan ended
    ...
    // Create CT-ICP factor
    // This factor connects two poses (t0 and t1) of the current scan
    ...
    auto ct_icp_factor = gtsam::make_shared<IntegratedCT_ICPFactor2D>(
      ...
      scan     // source scan with timestamps
    );
    ct_icp_factor->set_max_correspondence_distance(max_correspondence_distance_);
...
int main(int argc, char** argv)
{
  ...
  rclcpp::spin(std::make_shared<SlamWithCTICPNode>());
  rclcpp::shutdown();
  return 0;
}
