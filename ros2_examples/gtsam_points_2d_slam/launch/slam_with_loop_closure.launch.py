...
def generate_launch_description():
    ...
    # Config files
    slam_lc_params_file = os.path.join(
        pkg_gtsam_points_2d_slam,
        'config',
        'slam_with_loop_closure_params.yaml'
    )
    ...
    # SLAM with Loop Closure node
    slam_lc_node = Node(
        package='gtsam_points_2d_slam',
        executable='slam_with_loop_closure_node',
        name='slam_with_loop_closure_node',
        output='screen',
        parameters=[
            slam_lc_params_file,
            {'use_sim_time': use_sim_time}
