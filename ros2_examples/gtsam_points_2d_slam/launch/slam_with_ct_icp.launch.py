...
def generate_launch_description():
    ...
    # Config files
    slam_ct_icp_params_file = os.path.join(
        pkg_gtsam_points_2d_slam,
        'config',
        'slam_with_ct_icp_params.yaml'
    )
    ...
    # SLAM with CT-ICP node
    slam_ct_icp_node = Node(
        package='gtsam_points_2d_slam',
        executable='slam_with_ct_icp_node',
        name='slam_with_ct_icp_node',
        output='screen',
        parameters=[
            slam_ct_icp_params_file,
            {'use_sim_time': use_sim_time}
        ],
        remappings=[
            ('scan', '/scan'),
