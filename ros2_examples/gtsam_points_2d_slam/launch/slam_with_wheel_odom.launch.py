...
def generate_launch_description():
    ...
    # Config files
    slam_wo_params_file = os.path.join(
        pkg_gtsam_points_2d_slam,
        'config',
        'slam_with_wheel_odom_params.yaml'
    )
    ...
    # SLAM with Wheel Odometry node
    slam_wo_node = Node(
        package='gtsam_points_2d_slam',
        executable='slam_with_wheel_odom_node',
        name='slam_with_wheel_odom_node',
        output='screen',
        parameters=[
            slam_wo_params_file,
            {'use_sim_time': use_sim_time}
