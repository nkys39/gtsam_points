...
def generate_launch_description():
    ...
    # Config files
    slam_fixed_lag_params_file = os.path.join(
        pkg_gtsam_points_2d_slam,
        'config',
        'slam_with_fixed_lag_params.yaml'
    )
    ...
    # SLAM with Fixed-lag Smoothing node
    slam_fixed_lag_node = Node(
        package='gtsam_points_2d_slam',
        executable='slam_with_fixed_lag_node',
        name='slam_with_fixed_lag_node',
        output='screen',
        parameters=[
            slam_fixed_lag_params_file,
            {'use_sim_time': use_sim_time}
        ],
        remappings=[
            ('scan', '/scan'),
